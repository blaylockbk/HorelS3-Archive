...
def get_grib2(DATE, model, field, fxx, DIR, idx=True, png=True, PATH='default'):
    ...
    # HRRR Destination Path (same for directory and s3 bucket)
    PATH = '%s/%s/%s/' % (model, field, DATE.strftime('%Y%m%d'))
    
    # Looking for this file
    FILE = '%s.t%02d.wrf%sf%02d.grib2' % (model, DATE.strftime('%H'), field, fxx)

    # If the destination path does not exist, create it
    if not os.path.exists(DIR+PATH):
        os.makedirs(DIR+PATH)
    
    # If the file does not exists or if it is smaller than 50 MB (maybe the 
    # file is incomplete), then download it.
    if not os.path.isfile(DIR+PATH+FILE) or os.path.getsize(DIR+PATH+FILE) < 5*10e6:
        # Build the URL string we want to download HRRRv2+
        URL = NOMADS+FILE
        # Download and save the grib2 file and the .idx file
        print 'Downloading:', URL
        urllib.urlretrieve(URL, DIR+PATH+FILE)
        if idx:
