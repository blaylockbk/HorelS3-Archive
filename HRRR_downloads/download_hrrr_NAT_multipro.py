...
"""
Download operational HRRR (hrrr) NATIVE GRID files from NCEP NOMADS site.
This downloads a subsection of the file.
...
def download_hrrr_subsection(hour,
                             subregions = {'BRIANHEAD':{'latitude':37.716, 'longitude':-112.844, 'size':2.5}},
                             field='nat',
                             forecast=range(0, 19)):
    """
    ...
    Input:
        ...
        subregions - a dictionary of the subregion spesifications
            ...
            ['size']      - the size of the subregion to get. In degrees lat/lon,
                            we will get a box the size of 2*sizex2*size
    ...
    for f in forecast:
        ...
        # Now that we have the full native grid file, make subsections for each region
        for K in subregions:
            name = K
            latitude = subregions[K]['latitude']
            longitude = subregions[K]['longitude']
            subregion_size = subregions[K]['size']
        ...
        os.system('rm '+ OUTDIR+FileName) # Delete the large subregion now. The subregion files are deleted the next day.
